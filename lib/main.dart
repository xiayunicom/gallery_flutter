...
class TaskManager {
  ...
  final StreamController<String> _refreshEventController =
...
class _GalleryPageState extends State<GalleryPage> {
  ...
  void _showTaskList() {
    final now = DateTime.now(); // 新增：记录打开时间
    showDialog(
      context: context,
      barrierDismissible: false, // 修改为false
      builder: (ctx) => TapRegion(
        // 新增：包裹TapRegion
        onTapOutside: (_) {
          // Workaround for iPadOS bug: ignore taps within first 500ms
        ...
        MetaData(
          ...
          child: SizedBox(
            ...
            child: GestureDetector(
              ...
              child: Hero(
                tag: isSelectionMode ? "no-hero-$path" : imgUrl,
                child: Stack(
                  children: [
                    ClipRRect(
...
class _PhotoPreviewPageState extends State<PhotoPreviewPage>
  ...
  // ==========================================
  // 核心修改：使用 AnnotatedRegion 强制白色状态栏
  // ==========================================
  @override
  Widget build(BuildContext context) {
